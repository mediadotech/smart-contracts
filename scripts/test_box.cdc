...
pub fun main(size: UInt32, pickTo: UInt32, picks: [UInt32], truncate: Int): [AnyStruct] {
    let stockLabels = [
    ...
    let box = FanTopSerial.Box(size: size, pickTo: pickTo)
    let tester = Test.BoxTester(&box as &FanTopSerial.Box)
    for pick in picks {
        tester.pick(pick)
    }
    tester.truncate(limit: truncate)

    var stock = ""
    for chunk in box.getStock() {
